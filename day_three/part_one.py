...
def get_adjacent_part_numbers(
    ...
    for i in range(col_idx - 1, -1, -1):  # prefix with digits to left
        ...
        if (row_idx, i) in seen:
            raise Exception(
                f"Already seen number at coordinates ({row_idx}, {i}). Should have already seen since "
                f"connected to the digit at "
                f"coordinate ({row_idx}, {col_idx})."
            )
